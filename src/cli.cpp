...
DpiMode ParseDpiMode(const std::string& s) {
  if (s == "auto") return DpiMode::kAuto;
  if (s == "system") return DpiMode::kSystem;
  return DpiMode::kPerMonitorV2;
}

CropMode ParseCropMode(const std::string& s) {
  if (s == "window") return CropMode::kWindow;
  if (s == "client") return CropMode::kClient;
  if (s == "dwm-frame") return CropMode::kDwmFrame;
  if (s == "manual") return CropMode::kManual;
...
ParseResult ParseArgs(int argc, char** argv) {
  ...
  for (int i = 0; i < argc; ++i) out.raw_args.push_back(argv[i]);
  ...
  if (cmd == "cap") {
    ...
    if (sub == "windows") out.command = CommandType::kListWindows;
    else if (sub == "monitors") out.command = CommandType::kListMonitors;
    else {
  ...
  while (i < argc) {
    ...
    if (a == "--log-dir") {
      if (!NeedValue(i, argc, a, &r.error)) return r;
      ...
      if (!NeedValue(i, argc, a, &r.error)) return r;
      std::string v = argv[++i];
      if (v == "window") out.cap.target = TargetType::kWindow;
      else if (v == "screen") out.cap.target = TargetType::kScreen;
      else {
      ...
      CropRect c{};
      if (!ParseInt(argv[++i], &c.x) || !ParseInt(argv[++i], &c.y) || !ParseInt(argv[++i], &c.w) ||
          !ParseInt(argv[++i], &c.h)) {
      ...
      if (!ParseInt(argv[++i], &out.cap.pad.l) || !ParseInt(argv[++i], &out.cap.pad.t) ||
          !ParseInt(argv[++i], &out.cap.pad.r) || !ParseInt(argv[++i], &out.cap.pad.b)) {
        r.error = "invalid --pad";
        return r;
      }
  ...
  if (out.command == CommandType::kCap) {
    ...
    if (out.cap.target == TargetType::kWindow) {
      const bool has_window_target = out.cap.window_query.hwnd.has_value() || out.cap.window_query.pid.has_value() ||
                                     out.cap.window_query.foreground || out.cap.window_query.title.has_value() ||
      ...
      if (!out.cap.screen_query.monitor.has_value() && !out.cap.screen_query.virtual_screen) {
        r.error = "screen target needs --monitor or --virtual-screen";
        return r;
      }
    }
    if (out.cap.crop_mode == CropMode::kManual && !out.cap.crop_rect.has_value()) {
...
const char* TargetTypeName(TargetType t) { return t == TargetType::kWindow ? "window" : "screen"; }
...
std::string BuildHelpText() {
  ...
  oss << "screencap - Windows screenshot comparison CLI\n\n"
      ...
      << "  screencap cap --method dxgi-monitor --target screen --monitor primary --out a.png\n";
  return oss.str();
}
