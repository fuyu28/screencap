...
> [!WARNING]
> このリポジトリは Rust 版への移行に伴い開発を終了し、アーカイブされています。
> 本 C++ 実装には既知のセキュリティ上の問題があるため、新規利用や旧バイナリの使用は推奨しません。
> 詳細は [アーカイブ前セキュリティ調査](SECURITY_AUDIT.md) を参照してください。
...
- 開発・保守は終了しています
- Issue・Pull Request・セキュリティ修正は受け付けていません
- リポジトリは過去の実装を参照する目的でのみ公開しています
- 実用には移行先の Rust 版を使用してください

以下の内容は、アーカイブ時点の旧 C++ 実装に関する資料です。
