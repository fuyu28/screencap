...
Rect Intersect(const Rect &a, const Rect &b) {
  return Rect{std::max(a.left, b.left), std::max(a.top, b.top),
...
Rect ResolveCropRectScreen(CropMode mode, const std::optional<CropRect> &manual,
  ...
  case CropMode::kWindow:
    if (!window) {
      *err = ErrorInfo{"crop window requested but no window target",
                       "ResolveCropRectScreen", std::nullopt, std::nullopt};
      return {};
    }
    base = window->rect;
    break;
  case CropMode::kClient:
    if (!window) {
      *err = ErrorInfo{"crop client requested but no window target",
                       "ResolveCropRectScreen", std::nullopt, std::nullopt};
      return {};
    }
  ...
  case CropMode::kDwmFrame:
    if (!window) {
      *err = ErrorInfo{"crop dwm-frame requested but no window target",
                       "ResolveCropRectScreen", std::nullopt, std::nullopt};
      return {};
    }
  ...
  case CropMode::kManual:
    if (!manual.has_value()) {
      *err = ErrorInfo{"manual crop missing rect", "ResolveCropRectScreen",
                       std::nullopt, std::nullopt};
      return {};
    }
    base = Rect{manual->x, manual->y, manual->x + manual->w,
                manual->y + manual->h};
    break;
  }
  ...
  if (!IsValidRect(clipped)) {
    *err = ErrorInfo{"crop rect is empty after intersection",
                     "ResolveCropRectScreen", std::nullopt, std::nullopt};
    return {};
  }
  return clipped;
...
bool CropImageInPlace(const Rect &crop_screen_rect, ImageBuffer *img,
  ...
  if (!IsValidRect(c)) {
    *err = ErrorInfo{"crop does not overlap image", "CropImageInPlace",
                     std::nullopt, std::nullopt};
    return false;
  }
  ...
  for (int y = 0; y < nh; ++y) {
    const uint8_t *src =
        img->bgra.data() +
        static_cast<size_t>((y0 + y) * img->row_pitch + x0 * 4);
