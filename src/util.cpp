...
std::string Iso8601NowLocal() {
  ...
  std::ostringstream oss;
  oss << std::put_time(&local_tm, "%Y-%m-%dT%H:%M:%S") << '.' << std::setw(3)
      << std::setfill('0') << ms.count() << sign << std::setw(2)
      << std::setfill('0') << off_h << ':' << std::setw(2) << std::setfill('0')
      << off_m;
  return oss.str();
}
...
std::string BuildTimestampForFilename() {
  ...
  std::ostringstream oss;
  oss << std::put_time(&local_tm, "%Y%m%d_%H%M%S") << '_' << std::setw(3)
